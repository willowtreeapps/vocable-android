...
3. **One PR per issue.** For large/heavy parent tickets, break the work into sub-issues per discrete unit of work (`Part of #<parent>`) rather than landing the whole ticket as one PR — open each sub-issue with the same `/create-ticket` skill. PR body uses `.github/PULL_REQUEST_TEMPLATE.md` — `Part of #<parent> — closes #<issue>`, a `## What` section describing the actual diff, a `## Why it's safe` section with concrete verification evidence (build/test output, screenshots, before/after tables — not just an assertion), and `## Out of scope / follow-up` calling out what's deliberately deferred and to which issue. For sub-issues of a large ticket, PRs target the parent's own integration branch (e.g. `feature/voice-selection`), not `main`, until the whole parent ticket is ready. This keeps the parent ticket's comment thread as a running index of completed sub-issues rather than one sprawling, hard-to-review diff.
