...
/**
 * Prototype (#676) tracking source: MediaPipe FaceDetector (BlazeFace short-range) + CameraX -
 * the lighter-weight alternative to [MediaPipeFaceTracker] (FaceLandmarker). Returns a bare 2D
 * nose-tip keypoint, not a head-pose transform - weaker signal for head *rotation* than
 * FaceLandmarker/ARCore centerPose, but a single-stage, ~230KB model vs. FaceLandmarker's
 * two-stage + geometry-solve ~3.75MB pipeline, so it's the more hardware-friendly option worth
 * comparing directly rather than only documenting from the other branch. Ported from
 * prototype/mediapipe-facedetector. Dumb detection source only - no feel/smoothing tuning here,
 * that lives in FaceTrackingViewModel.onFaceDetectorUpdate.
 */
class MediaPipeFaceDetectorTracker(
    context: Context,
    ...
    private val faceDetector: FaceDetector = FaceDetector.createFromOptions(
        ...
        FaceDetector.FaceDetectorOptions.builder()
            ...
            .setErrorListener { error -> Timber.e(error, "MediaPipeFaceDetectorTracker detection error") }
            .build()
    )
        ...
        cameraProviderFuture.addListener(
            {
                ...
                try {
                    ...
                    Timber.e(e, "MediaPipeFaceDetectorTracker camera bind failed")
                }
            },
